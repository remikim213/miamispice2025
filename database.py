...
class DatabaseManager:
    ...
    def get_restaurant_id_field(self, collection_name):
        ...
        if sample_doc:
            ...
            for key in sample_doc.keys():
                if 'id' in key.lower() and key != '_id':
    ...
    def get_filter_data(self):
        ...
        if restaurant_data:
            ...
            restaurants = sorted([name for name in data.get('restaurants', []) if name])
            cuisines = sorted([cuisine for cuisine in data.get('cuisines', []) if cuisine])
            locations = sorted([location for location in data.get('locations', []) if location])
        else:
        ...
        existing_days = [doc['_id'] for doc in self.options_collection.aggregate(days_pipeline) if doc['_id']]
        days = [day for day in Config.DAY_ORDER if day in existing_days]
        
        existing_times = [doc['_id'] for doc in self.options_collection.aggregate(times_pipeline) if doc['_id']]
        ...
        users = sorted([doc['_id'] for doc in self.reviews_collection.aggregate(users_pipeline) if doc['_id']])
    ...
    def search_restaurants(self, filters):
        ...
        if filters.get('search_name') and filters['search_name'] != "All":
            filter_query['Name'] = {'$regex': filters['search_name'], '$options': 'i'}
        if filters.get('selected_cuisine') and filters['selected_cuisine'] != "All":
        ...
        restaurant_ids = [restaurant[restaurant_id_field] for restaurant in matching_restaurants]
        ...
        if filters.get('selected_day') != "All" or filters.get('selected_time') != "All":
            ...
            if filters.get('selected_day') != "All":
                options_filter['Day'] = filters['selected_day']
            if filters.get('selected_time') != "All":
    ...
    def get_restaurant_options(self, restaurant_id, selected_day="All", selected_time="All"):
        ...
        restaurant_options.sort(key=lambda x: (
            Config.DAY_ORDER.index(x.get('Day', '')), 
            Config.TIME_ORDER.index(x.get('Time', ''))
        ))
    ...
    def submit_review(self, restaurant_name, user_name, rating, comment):
        ...
        restaurant_id = restaurant_doc[restaurant_id_field]
    ...
    def get_user_reviews(self, user_name):
        ...
        pipeline = [
            {'$match': {'UserName': {'$regex': user_name.strip(), '$options': 'i'}}},
            {'$sort': {'CreatedAt': -1}},
            {
                '$lookup': {
        ...
        for review in self.reviews_collection.aggregate(pipeline):
            ...
            enriched_reviews.append({
                'Restaurant': review.get('Restaurant', 'Unknown'),
